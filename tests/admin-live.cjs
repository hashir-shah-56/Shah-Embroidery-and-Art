  ...
  try {
    ...
    for (const [suffix, imageUrl] of [['empty-image', ''], ['external-image', 'https://example.com/product.jpg']]) {
      ...
      const result = await page.evaluate(async ({ title, imageUrl }) => {
        const { data, error } = await supabaseClient.from('products').insert({ title, category: 'Hand Embroidery', price: 1, image_url: imageUrl, is_custom_quote: false, in_stock: false }).select('id').single();
        return { data, error };
