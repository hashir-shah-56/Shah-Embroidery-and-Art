...
const budgetRanges=['Under Rs. 2,000','Rs. 2,000 - 5,000','Rs. 5,000 - 10,000','Rs. 10,000+'];
assert.deepEqual(await page.locator('input[name=customBudget]').evaluateAll(inputs=>inputs.map(input=>input.value)),budgetRanges);
assert.deepEqual(await page.locator('.custom-order-budget-grid label').allTextContents(),budgetRanges);
await page.locator('label[for=budgetMid]').click();
...
let rows=await db();assert.equal(rows.length,1);assert.equal(rows[0].budget_range,'Rs. 2,000 - 5,000');assert.equal(rows[0].user_id,null);assert.equal(rows[0].guest_email,'test@example.com');assert.equal(rows[0].reference_image_urls.length,1);assert.equal(await page.evaluate(()=>fixture.uploads),2);assert.equal(await page.locator('#customDetails').inputValue(),'');
...
assert.match(await page.locator('#adminCustomOrderDetails-2').innerText(),/Rs\. 2,000 - 5,000/);
console.log('PASS: all four PKR budget labels/values, selected range in INSERT payload and owner request details');
