...
### 5. Featured & Latest Artwork Grids (`#featured`)
...
- **Description:** Showcase of handcrafted artwork pieces available for purchase or custom replication. Each card displays title, category tag, price tag, quick view trigger, and detail links.
- **Interactive Behavior:**
...
The `products` table now powers the Featured & Latest Work section on `index.html`. `js/product-loader.js` fetches up to six rows with `is_featured=true` and `in_stock=true`, newest first. Existing Featured content remains if the query fails or returns no rows.
...
- Supports adding, editing, cancelling edits, and confirmation-protected deletion. All products, including out-of-stock and non-featured rows, are accessible in a compact table with 25 rows per page. Featured, in-stock changes appear on the homepage's next load.
